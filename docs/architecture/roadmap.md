...
## 3.7 服务闭环规划（2026-08-05 外部评审十项的取舍，未排期）

评审框架采纳：不再无止境加意图，把系统补成五个闭环——
**会话理解 → 服务交付 → 人工协同 → 主动经营 → 运营优化**。
十项建议对照仓库现状分三档（排期时再写阶段文档）：

### 建议立项（按优先级）

| 候选阶段 | 方向 | 现状与 v1 范围 |
|---|---|---|
| Stage 34 | **Case 工单与 SLA 治理**（最高优先） | 真缺口：`chat_handoff_ticket` 是会话内人工队列，不是跨会话/跨渠道的长期服务记录。v1：Case 实体（9 态生命周期）+ 自动创建（转人工/执行失败/连击 UNKNOWN 收口到 Case）+ 跨会话关联（related_task/order/session）+ SLA 时限与超时升级（cron 复用 deploy/scheduler）+ resolution code。**Service Recovery 并入本阶段**：触发面大半已有（EXECUTION_FAILED 建单/unknown 连击/护栏工单），缺的只是结构化补偿政策（config 化 + 资格 reason_codes + 补偿恒走确认门与唯一写入口，LLM 不得自由决定补偿——评审红线与既有纪律同构） |
| Stage 35 | **身份核验分级（IAL）** | 便宜且是安全轴收口：工具目录（post-27 单一事实来源）加 `required_identity_level` 元数据 + 调用前校验；会话归属校验已有，等级推导 v1 = 鉴权态映射（AUTH_ENABLED→IAL1，OTP/二次验证接真实渠道后补）；敏感工具（改地址/导出个人数据）声明 IAL2+，未达标转人工。防 OTP 入日志沿用脱敏纪律 |
| Stage 36 | **事件驱动主动客服** | NBA 的另一半：抑制矩阵/频控/冷却/opt-out/主动消息 API 全部可复用，缺的只是事件入口（webhook/Outbox 表）+ 幂等路由 + 发送前重查最新事实。评审红线（事件幂等/只通知一次/退订/发送依据留痕）与 Stage 31 纪律同构，mock 事件即可打通 |
| Stage 37 | **知识缺口发现 + 规则版质量 Scorecard** | 原料已全在决策日志（RAG 拒答/低分/改写失败/二判后转人工/差评/FAQ 命中仍不满意），`kb_quality_queries` 已有盲区 SQL——升级为缺口报表脚本（聚类+候选问题+FAQ 草稿进运营后台草稿态，闭环到 Stage 16 审核流）。质量 Scorecard v1 用规则从决策日志离线算（错误切换/重复询问/营销干扰/确认门遵守均可推导）；**LLM 自动评分 + 三标签（自动/人工/用户）留真实 Key 后，且自动分不作唯一真值**（评审红线采纳） |

### 并入既有面（不独立立项）

- **人工协作升级**：上下文移交包已有（context_json），随 Stage 34 扩字段
  （customer_goal/confirmed_facts/recommended_next_step）；Agent Assist
  坐席草稿建议默认关、待真实坐席使用后开（AI 建议≠已执行，写操作仍走确认门）。

### Backlog（前置条件未满足，暂不做）

| 方向 | 前置 | 备注 |
|---|---|---|
| VoC 根因分析/主题聚类 | 真实流量 | 合成数据上聚类是表演；决策日志字段已够未来直接建 |
...
转人工五类触发、上下文移交包、双端实时 WS、坐席 API、拒绝冷却/永久退订、
营销与服务分离（全局抑制矩阵）、主动消息 API、决策日志逐轮留痕。
